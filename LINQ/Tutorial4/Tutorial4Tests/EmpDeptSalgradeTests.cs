...
public class EmpDeptSalgradeTests
{
    ...
    public void ShouldReturnDept30EmpsOrderedBySalaryDesc()
    {
        ...
        List<Emp> result = emps.Where(x => x.DeptNo == 30).OrderByDescending(x => x.Sal).ToList(); 
    ...
    public void ShouldReturnEmployeesFromChicago()
    {
        ...
        var deptNos = depts.Where(dept => dept.Loc == "CHICAGO").Select(dept => dept.DeptNo);

        List<Emp> result = emps.Where(x => deptNos.Contains(x.DeptNo)).ToList(); 
    ...
    public void ShouldSelectNamesAndSalaries()
    {
        ...
        var result = emps.Select(x => (x.EName, x.Sal));
    ...
    public void ShouldJoinEmployeesWithDepartments()
    {
        ...
        var result = emps.Join(depts, emp => emp.DeptNo, dept => dept.DeptNo, (emp, dept) => (emp.EName, dept.DName));
    ...
    public void ShouldCountEmployeesPerDepartment()
    {
        ...
        var result = emps.GroupBy(x => x.DeptNo).Select(x => (DeptNo: x.Key, Count: x.Count()));
    ...
    public void ShouldReturnEmployeesWithCommission()
    {
        ...
        var result = emps.Where(x => x.Comm != null).Select(x => (x.EName, x.Comm));
    ...
    public void ShouldCalculateAverageSalaryPerDept()
    {
        ...
        var result = emps.GroupBy(x => x.DeptNo).Select(x => (deptno: x.Key, avgsal: x.Average(emp => emp.Sal)));
